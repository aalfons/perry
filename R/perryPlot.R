...
perryPlot.perry <- function(object, method = c("box", "density", "dot"),
                            select = NULL, seFactor = NA, ...) {
    # initializations
    if (object$splits$R == 1) method <- "dot"
    else method <- match.arg(method)
    # extract data for plotting
    reps <- method %in% c("box", "density")
    setup <- setupPerryPlot(object, select = select, reps = reps,
                            seFactor = seFactor)
...
perryPlot.setupPerryPlot <- function(object,
                                     method = c("box", "density", "dot", "line"),
                                     mapping = NULL, facets = object$facets,
                                     ...) {
    # initializations
